      ...
      {safe.safeShare !== null && (
        <div className="flex flex-col gap-1.5 border border-surface-2 px-3.5 py-3">
          <p className="text-sm leading-relaxed text-foreground">
            <span className="font-mono text-lg font-semibold text-positive tabular-nums">
              {pct(safe.safeShare)}
            </span>{" "}
      ...
      {safe.buffered.length > 0 && (
        <ul className="flex flex-col">
          {safe.buffered.map((node) => (
            <li
              key={node.id}
              className="flex flex-col gap-1 border-t border-surface-2 px-1 py-2.5"
            >
              ...
              <span className="font-mono text-[10px] tracking-[0.1em] text-positive uppercase">
