...
import type { GraphData, GraphEdge, GraphNode, Source } from "@/lib/api/types";
import { edgeRowsFor, fieldRowsFor, pretty, type FieldRow } from "@/lib/provenance/fields";
...
function useGraphLookups(graph: GraphData) {
  return useMemo(() => {
...
/** Evidence panel: every attested field of the selection, with its provenance. */
export function EntityPanel({ selection, graph, onClose }: EntityPanelProps) {
  const { sources, resolveName } = useGraphLookups(graph);

  const isNode = selection.kind === "node";
  const title = isNode ? selection.node.name : pretty(selection.edge.type);
  const badge = isNode
    ? KIND_STYLE[selection.node.kind]
    : {
        label: selection.edge.derived ? "derived edge" : "relationship",
        badge: "bg-zinc-200 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300",
      };
  const description =
    isNode && "description" in selection.node.entity ? selection.node.entity.description : null;
  const rows: FieldRow[] = isNode
    ? fieldRowsFor(selection.node, resolveName)
    : edgeRowsFor(selection.edge, resolveName);
  ...
  return (
    <aside className="flex w-96 shrink-0 flex-col overflow-y-auto border-l border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-950">
      ...
      <p className="mb-3">
        <span className={`rounded px-1.5 py-0.5 text-[11px] ${badge.badge}`}>{badge.label}</span>
      </p>
      ...
      <dl className="space-y-3">
        {rows.map((row, index) => (
          <div key={`${row.label}-${index}`}>
            ...
            <dd className="text-xs leading-5 whitespace-pre-wrap text-zinc-800 dark:text-zinc-200">
              {row.value}
            </dd>
